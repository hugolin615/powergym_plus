...
class LoadProfile:
    def __init__(self, steps, dss_folder_path, dss_file, worker_idx=None):
        ...
        self.dss_folder_path = dss_folder_path #'/home/romesh-prasad/ONR/volt_var/volt_var/systems/13Bus'
        print(dss_folder_path)
        self.loadshape_path = os.path.join(dss_folder_path, 'loadshape') #'/home/romesh-prasad/ONR/volt_var/volt_var/systems/13Bus/loadshape'
    ...
    def find_load_names(self, main_dss):
        ...
        def find_load_name(fname, names):
            file_path = os.path.join(self.dss_folder_path, fname) #'/home/romesh-prasad/ONR/volt_var/volt_var/systems/13Bus/IEEE13Nodeckt_daily.dss'
            assert os.path.exists(file_path), file_path + ' not found'
            
            needs_load_daily, daily_mode = False, False 
            with open(file_path, 'r') as fin: #this part of code reads through IEEE13Nodeckt_daily.dss
                for line in fin:
                    low = line.strip().lower() #lowercase
                    if low.startswith('new load.'):
                        if 'daily' in low:
                            spt = line.split(' ')
                            spt = list(filter(None, spt)) # filter out the empty string
                            names.append(spt[1].split('.',1)[1]) #add loadnames to the list
